...
internal sealed class FinancialMetricsService : IFinancialMetricsService
{
        ...
        DateOnly from, DateOnly to, CancellationToken ct = default)
    {
        ...
        var rows = await _db.ProcessedExpenses
            .AsNoTracking()
        ...
        DateOnly from, DateOnly to, CancellationToken ct = default)
    {
        ...
        var grouped = await _db.ProcessedExpenses
            .AsNoTracking()
            .Where(e =>
                e.EffectiveDate >= fromUtc &&
                e.EffectiveDate <= toUtc &&
                e.FinancialImpact == FinancialImpact.RealExpense)
            .GroupBy(e => new
            {
                e.CategoryId,
        ...
        int months, CancellationToken ct = default)
    {
        ...
        return rows
            ...
            .Select(g =>
            {
                var expenses = g.Where(r => r.FinancialImpact == FinancialImpact.RealExpense).Sum(r => r.TotalAmount);
        ...
        int year, int month, CancellationToken ct = default)
    {
        ...
        var prevTo = new DateOnly(prevDate.Year, prevDate.Month, DateTime.DaysInMonth(prevDate.Year, prevDate.Month));

        // Una sola query para los dos meses
        ...
        // Variación por categoría — solo RealExpense
        var currByCat = currentRows
            .Where(r => r.Impact == FinancialImpact.RealExpense)
        ...
        {
            var (name, curr) = currByCat.TryGetValue(id, out var c) ? c : (prevByCat.TryGetValue(id, out var p) ? p.Item1 : "?", 0m);
            var prev = prevByCat.TryGetValue(id, out var pv) ? pv.Item2 : 0m;
            if (currByCat.TryGetValue(id, out var cv)) curr = cv.Item2;
        ...
        return new MonthComparison(currentSummary, previousSummary, expVariation, expVariationPct,
    ...
    {
        var expenses = rows.Where(r => r.Impact == FinancialImpact.RealExpense).Sum(r => r.Amount);
        ...
        return new PeriodSummary(from, to, income, expenses, net, (decimal)savings,
            rows.Count,
            rows.Count(r => r.Status == ExpenseStatus.Confirmed),
            rows.Count(r => r.Status == ExpenseStatus.Reviewed),
            currency);
    }
    ...
    // Tipos auxiliares internos para evitar tipos anónimos en múltiples métodos
    private sealed record RawRow(decimal Amount, FinancialImpact Impact, ExpenseStatus Status, string Currency);
    private sealed record CompareRow(DateTime Date, decimal Amount, FinancialImpact Impact, ExpenseStatus Status,
        string Currency, string CategoryDisplay, Guid CategoryId);
}