...
namespace FinancialSystem.Application.Reconciliation.Commands
{
        ...
        string? Error = null)
    {
        ...
        public static ConfirmGroupResult Ok(Guid expenseId, decimal refTotal, decimal candTotal, decimal delta) =>
            new(true, expenseId, refTotal, candTotal, delta);
    }
    ...
    public sealed class ConfirmGroupHandler
    {
        ...
        public async Task<ConfirmGroupResult> Handle(ConfirmGroupCommand cmd, CancellationToken ct)
        {
            ...
            if (cmd.CategoryId == Guid.Empty)
                return ConfirmGroupResult.Failure("CategoryId es requerido");
            ...
            // ── Validar que ninguno esté ya procesado ────────────────────────────
            var alreadyError = await ValidateNotAlreadyProcessedAsync(refs, cands, ct);
            ...
            var expense = new ProcessedExpense
            {
                ...
                ProcessingSource = ProcessingSource.ManualMatch,
                MatchScore = null,
                AmountDelta = delta,
                CreatedAt = now,
                ProcessedAt = now,
                ProcessedBy = cmd.ConfirmedBy,
            };
            ...
            return ConfirmGroupResult.Ok(expense.Id, refTotal, candTotal, delta);
        }
            ...
            CancellationToken ct)
        {
            ...
            var txIds = items.Where(i => i.Source == MovementSource.CreditCard).Select(i => i.Id).ToHashSet();
            var bsIds = items.Where(i => i.Source == MovementSource.BankDebit).Select(i => i.Id).ToHashSet();
            var meIds = items.Where(i => i.Source is MovementSource.ManualDynamic or MovementSource.ManualFixed)
                             .Select(i => i.Id).ToHashSet();

            var txMap = txIds.Count > 0
                ? (await _db.Transactions.AsNoTracking().Where(t => txIds.Contains(t.Id)).ToListAsync(ct))
                  .ToDictionary(t => t.Id, t => MovementAdapter.FromTransaction(t))
                : new Dictionary<Guid, FinancialMovement>();

            var bsMap = bsIds.Count > 0
                ? (await _db.BankStatements.AsNoTracking().Where(b => bsIds.Contains(b.Id)).ToListAsync(ct))
                  .ToDictionary(b => b.Id, b => MovementAdapter.FromBankStatement(b))
                : new Dictionary<Guid, FinancialMovement>();

            var meMap = meIds.Count > 0
                ? (await _db.ManualExpenses.AsNoTracking().Where(m => meIds.Contains(m.Id)).ToListAsync(ct))
        ...
        private async Task<string?> ValidateNotAlreadyProcessedAsync(
            ...
            CancellationToken ct)
        {
            var txRefIds = refs.Where(m => m.Source == MovementSource.CreditCard).Select(m => m.Id).ToList();
            var bsRefIds = refs.Where(m => m.Source == MovementSource.BankDebit).Select(m => m.Id).ToList();
            var meCandIds = cands.Where(m => m.Source is MovementSource.ManualDynamic or MovementSource.ManualFixed)
                                 .Select(m => m.Id).ToList();

            var conflicts = new List<Guid>();
            if (txRefIds.Count > 0)
                conflicts.AddRange(await _repository.GetAlreadyProcessedSourceIdsAsync(
            ...
            return $"Movimientos ya procesados: {string.Join(", ", conflicts.Take(5))}" +
                   (conflicts.Count > 5 ? $" (y {conflicts.Count - 5} más)" : string.Empty);
        }

        private static string BuildDescription(List<FinancialMovement> refs, List<FinancialMovement> cands)
        {
            if (refs.Count == 1 && cands.Count == 1)
            {
                var r = refs[0].Description.Trim();
                var c = cands[0].Description.Trim();
                return r.Equals(c, StringComparison.OrdinalIgnoreCase) ? r : $"{c} ({r})";
            }
            var primary = cands.Count == 1 ? cands[0].Description.Trim() : refs[0].Description.Trim();
            return $"{primary} [{refs.Count}↔{cands.Count}]";
        }

        private static ProcessedExpenseItem BuildItem(FinancialMovement m, ReconciliationItemRole role) =>
            new()
            {
