...
// independientemente de su fuente (banco, tarjeta, manual).
// La conciliación opera exclusivamente sobre este modelo — nunca
// sobre Transaction directamente.
//
// DECISIÓN DE DISEÑO: record inmutable en lugar de clase mutable.
// La conciliación no modifica movimientos, sólo los cruza.
// ════════════════════════════════════════════════════════════════════
...
public sealed record FinancialMovement
{
    ...
    /// <summary>Categoría semántica del movimiento (puede ser Unknown si no se pudo inferir).</summary>
    public MovementCategory Category { get; init; } = MovementCategory.Unknown;

    /// <summary>
    /// Método de pago declarado. Sólo disponible en movimientos manuales.
    ...
    /// <summary>Nombre de hoja (para movimientos Excel).</summary>
...
{
    BankDebit,       // Excel débito bancario
    CreditCard,      // PDF tarjeta (Visa, Mastercard, etc.)
    ManualDynamic,   // Excel manual hoja "Gastos dinámicos"
    ManualFixed,     // Excel manual hoja "Gastos fijos"
}
