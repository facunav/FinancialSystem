...
public sealed record ProcessedExpenseDto(
    ...
    string? ProcessedBy,
    double? MatchScore,
    IReadOnlyList<ProcessedItemDto> ReferenceItems,
    IReadOnlyList<ProcessedItemDto> CandidateItems)
{
    public static ProcessedExpenseDto From(ProcessedExpense e) => new(
        ...
        e.Status.ToString(),
        e.ProcessingSource.ToString(),
        e.ProcessedAt,
        e.ProcessedBy,
        e.MatchScore,
        e.Items.Where(i => i.Role == ReconciliationItemRole.Reference)
               .Select(ProcessedItemDto.From).ToList(),
