    ...
    public static class ReconciliationEndpoints
    {
            ...
            CancellationToken ct)
        {
            ...
            var batchResult = await confirmationService.ConfirmBatchAsync(
                orderedRequests, request.ConfirmedBy, ct);
            ...
            foreach (var req in orderedRequests)
            {
                ...
                if (successQueue.Count > 0) successByPair[key] = successQueue.Dequeue();
            }

            // Paso 5: construir resultados en orden original
            ...
            CancellationToken ct)
        {
            ...
            var processedTx = (await processedRepo.GetAlreadyProcessedSourceIdsAsync(
                SourceEntityType.Transaction, txIds, ct)).ToHashSet();
            var processedBs = (await processedRepo.GetAlreadyProcessedSourceIdsAsync(
                SourceEntityType.BankStatement, bsIds, ct)).ToHashSet();
            var processedExp = (await processedRepo.GetAlreadyProcessedSourceIdsAsync(
                SourceEntityType.ManualExpense, expIds, ct)).ToHashSet();
