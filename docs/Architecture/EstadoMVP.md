...
- ~~**`docs/UX/ClassificationUX.md`** → Actualizar §1.2 (describe un `<select>` editable de cuenta que ya es de solo lectura) y §1.5 (dice que "Cuentas" no existe, cuando `accounts.html` ya existe).~~ **Hecho (PATCH-032)**: sección "Movimientos" (cuenta financiera de solo lectura + resolución automática ya implementada) y sección "Cuentas" (`accounts.html` ya no es pantalla futura) corregidas; de paso se documentaron 3 controles del modal de clasificación que no tenían mención en el documento (precarga OwnCard/DebtPayment, sugerencia de período financiero, alta rápida de contraparte).
