...
**Estado:** Aceptado (documento arquitectónico de visión y principios — ninguna fase de esta ADR está implementada. La Fase 1 vigente hoy, ver ADR-006, es puramente de lectura y no tiene memoria propia). Reemplaza la versión anterior de este mismo ADR con un modelo conceptual más preciso; no contradice sus principios centrales.
...
Modelo conceptual de estados por los que atraviesa una investigación — sin definir aún transiciones exactas ni quién puede dispararlas, eso corresponde a cuando se implemente la Fase 3 (§8):

* **Abierta** — la investigación se creó, todavía no tiene desarrollo.
* **En progreso** — tiene desarrollo (uno o más hallazgos) pero todavía no llegó a una conclusión.
* **Resuelta** — llegó a una conclusión.
* **Descartada** — se determinó que no amerita seguir investigándose, sin haber llegado a una conclusión.
...
* **AuditTools** (`FindSuspiciousMovements`, `FindMisclassifiedMovements`): hoy son reglas objetivas fijas, sin memoria ni IA, y eso no cambia con esta ADR. Recién en la Fase 5 (§8) el historial acumulado de investigaciones podrá alimentar nuevas señales de auditoría — como sugerencia para revisión humana, nunca como corrección automática.
...
Ollama nunca consulta la base de datos directamente, en ninguna fase. Toda información que Ollama usa para razonar le llega exclusivamente a través de tools del MCP — el mismo principio que ya rige para el MCP en general. La memoria, cuando exista, es una fuente de contexto más entre las que Ollama recibe a través de tools: agrega antecedentes (por ejemplo, que un caso similar ya se investigó y qué se concluyó), pero no reemplaza ni modifica el razonamiento del modelo. Ollama sigue decidiendo qué hacer con esa información, igual que con el resultado de cualquier otra tool.

## 8. Roadmap

* **Fase 2 — Persistencia de investigaciones.** Las investigaciones (§3) dejan de vivir solo en la conversación y empiezan a persistir entre sesiones.
* **Fase 3 — Tools para crear, actualizar y consultar investigaciones.** El MCP expone tools para registrar una investigación nueva, agregarle hallazgos, cambiar su estado (§5) y consultarla — por movimiento referenciado o por búsqueda libre.
* **Fase 4 — Integración con Ollama.** Ollama empieza a poder usar la memoria como contexto adicional (§7), sobre información que ya le llega a través de tools.
* **Fase 5 — Auditorías inteligentes basadas en memoria.** Nuevas señales de auditoría que aprovechan el historial acumulado de investigaciones (§6, AuditTools) — siempre como sugerencia para revisión humana, nunca como corrección automática.

## 9. Consecuencias

* El MCP sigue sin escribir datos financieros en ninguna fase de esta ADR — eso no cambia cuando el MCP empiece a tener memoria propia.
