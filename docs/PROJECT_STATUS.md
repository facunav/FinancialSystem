...
| **Servidor MCP** (`hosts/FinancialSystem.McpServer`) | Exponer el sistema como herramientas MCP para un cliente de IA (Claude, etc.) | Terminado / funcional | Alta, con un catálogo de herramientas (`ToolRegistry`) parcialmente desincronizado con las tools reales | Application, Infrastructure |
...
| **ADR-007** | Memoria del MCP (investigaciones) | Vigente (declaración de estado errónea) | El diseño se respeta y está bien implementado (Fases 2-4: persistencia, tools CRUD, integración con Ollama), pero el documento declara explícitamente "ninguna fase implementada" — falso, y es la discrepancia doc/código más flagrante del repositorio. Corregir con prioridad alta. |
