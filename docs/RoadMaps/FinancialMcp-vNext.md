...
4. **No existe el concepto de Cuenta (`FinancialAccount`).** `BankStatement` tiene `BankName` como texto libre; `Transaction` (tarjeta) no tiene ningún campo que identifique de qué tarjeta vino. Sin esto, "Banco + Tarjetas + Cuentas financieras" como fuentes explícitas no es representable en los datos.
